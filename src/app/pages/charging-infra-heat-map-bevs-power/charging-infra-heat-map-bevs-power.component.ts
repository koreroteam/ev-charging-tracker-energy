...
@Component({
  selector: 'ngx-charging-infra-heat-map-bevs-power',
  templateUrl: './charging-infra-heat-map-bevs-power.component.html',
  styleUrls: ['./charging-infra-heat-map-bevs-power.component.scss']
})
export class ChargingInfraHeatMapBevsPowerComponent implements OnInit, AfterViewInit {
  ...
  private createMap(): void {
    ...
    document.getElementById('heatMapContainerBEVsPower').style.backgroundColor = "rgba(85,90,96,0.3)";
  
    this.map = L.map('heatMapContainerBEVsPower', {
  ...
  private async renderKreiseMap(): Promise<void> {
    ...
    const rates = Object.values(chargePointAreas).map(({ total_power, gesamt_bevs }) =>
    Number((total_power / gesamt_bevs).toFixed(2)) );
    rates.sort((a, b) => a - b);
    ...
    this.http.get(kreiseDataUrl).subscribe((data: any) => {
      ...
      this.geoJsonLayer = L.geoJSON(data, {
        style: (feature) => {
          ...
          const count = chargePointData["total_power"];
        ...
        //  console.log(color)
          ...
          onEachFeature: (feature, layer) => {
            ...
            const popupContent = `Landkrise: ${kreiseName}<br>Leistungen der Ladepunkte: ${count}<br>Anzahl der BEVs: ${gesamt_bevs}<br>LadeLeistung per BEVs: ${(count / gesamt_bevs).toFixed(2)}`;
  ...
  private getFillColorKreise(kreiseCount:number):string{
    ...
    } else if (kreiseCount > 6) {
      return 'rgba(175, 209, 231, 1)'; 
    } else if (kreiseCount > 4) {
      return 'rgba(62, 142, 196, 1)'; 
    } else if (kreiseCount> 2) {
...
private convertToCSV(data: any[], headerLabel: string): string {
  const header = [headerLabel,'Ladeleistungen der Ladepunkte', 'Anzahl der BEVs','LadeLeistung per BEVs'];
  let csvRows = [];
