...
@Component({
  selector: 'ngx-charging-infra-heat-map-population',
  templateUrl: './charging-infra-heat-map-population.component.html',
  styleUrls: ['./charging-infra-heat-map-population.component.scss']
})
export class ChargingInfraHeatMapPopulationComponent implements OnInit, AfterViewInit {
  ...
  private createMap(): void {
    ...
    document.getElementById('LadepunktePopulation').style.backgroundColor = "rgba(85,90,96,0.3)";
    this.map = L.map('LadepunktePopulation', {
  ...
  private async renderKreiseMap(): Promise<void> {
   ...
   const densities = Object.entries(chargePointsByKreise).map(([name, count]) => {
    const areaPopulation = this.DISTRICT_Population_Kreise.get(name) ?? 1;
    return (typeof count === 'number' ? count : Number(count)) / areaPopulation;  
  }).sort((a, b) => a - b);
  ...
  this.http.get(kreiseDataUrl).subscribe((data: any) => {
        ...
        this.geoJsonLayer = L.geoJSON(data, {
          style: (feature) => {
            ...
            const chargePointDensity = chargePoints / areaPopulation;
          ...
          onEachFeature: (feature, layer) => {
            ...
            const areaPopulation = this.DISTRICT_Population_Kreise.get(kreiseName) ?? 1; 
            const chargePointDensity = chargePoints / areaPopulation;
            this.popupDataKreise.push([kreiseName, chargePoints, areaPopulation,  Number(chargePointDensity.toFixed(2))]);

  
            const popupContent = `Landkreis: ${kreiseName}<br>Anzahl Ladepunkte: ${chargePoints}<br>Einwohner je km²: ${areaPopulation}<br>Ladepunkte je Einwohner/km²: ${chargePointDensity.toFixed(2)}`;
  ...
  private renderRegierungsbezirkMap(): void {
    ...
    this.apiService.getChargePointsByDistricts().then((chargePointsByDistricts) => {
      const densities = Object.entries(chargePointsByDistricts)
      .filter(([name]) => this.DISTRICT_Population.has(name))
      .map(([name, count]) => {
          const populationSize = this.DISTRICT_Population.get(name) ?? 1;
          return (typeof count === 'number' ? count : Number(count)) / populationSize;  
      })
      .sort((a, b) => a - b);
  ...
  console.log(thresholds)
      ...
      //  console.log('Charge Points By Districts:', chargePointsByDistricts);
        ...
        this.geoJsonLayer = L.geoJSON(mergedGeoJSON, {
          style: (feature) => {
            ...
            const chargePointDensity = count /  areaPopulation;
            this.popupDataRegierungsbezirk.push([name, count, areaPopulation,  Number(chargePointDensity.toFixed(2))]);
            
         //   console.log(chargePointDensity)
          ...
          onEachFeature: (feature, layer) => {
            ...
            const popupContent = `Regierungsbezirk: ${name}<br>Anzahl Ladepunkte: ${count}<br>Einwohner je km²: ${areaPopulation}<br>Ladepunkte je Einwohner/km²: ${chargePointDensity.toFixed(2)}`;
...
["Kreisfreie Stadt Bamberg", 1423],
 ...
 ['Kreisfreie Stadt Leipzig', 2021],
  ...
  //   if(regierungsbezirkCount > 39.99){
  //     return 'rgba(255, 0, 0, 0.9)';
  //   }else if (regierungsbezirkCount > 29.99) {
  //     return 'rgba(255, 0, 0, 0.7)'; 
  //   }else if(regierungsbezirkCount > 19.99){
  //     return 'rgba(255, 0, 0, 0.5)'; 
  //   }
  //    else if (regierungsbezirkCount > 9.99) {
  ...
  //   if(kreiseCount > 2.99){
  //     return 'rgba(255, 0, 0, 0.9)';
  //   } else if (kreiseCount > 1.99) {
  //     return 'rgba(255, 0, 0, 0.7)'; 
  //   } else if (kreiseCount > 0.99) {
  //     return 'rgba(255, 0, 0, 0.5)'; 
  //   } else if (kreiseCount> 0.499) {
  //     return 'rgba(255, 0, 0, 0.3)';
  //   } else {
  //     return 'rgba(255, 0, 0, 0.1)'
  //   }
...
private convertToCSV(data: any[], headerLabel: string): string {
  const header = [headerLabel, 'Anzahl Ladepunkte', 'Population je km^2', 'Ladepunkte je Population je km^2'];
  let csvRows = [];
