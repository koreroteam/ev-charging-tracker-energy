...
@Component({
  selector: 'ngx-strom-erzeuger-strom-speicher-total-number',
  templateUrl: './strom-erzeuger-strom-speicher-total-number.component.html',
  styleUrls: ['./strom-erzeuger-strom-speicher-total-number.component.scss']
})
export class StromErzeugerStromSpeicherTotalNumberComponent implements OnInit, AfterViewInit {
  ...
  private createMap(): void {
    ...
    document.getElementById('heatMapContainerStromSpeicher').style.backgroundColor = "rgba(85,90,96,0.3)";
    this.map = L.map('heatMapContainerStromSpeicher', {
  ...
  private renderMapZipCode({chargePointsData, thresholds}: {chargePointsData: any, thresholds: number[]}, data: any): void {
    ...
    this.geoJsonLayer = L.geoJSON(data, {
      ...
      onEachFeature: (feature, layer) => {
        ...
        const popupContent = `PLZ: ${zipcode}<br>Anzahl Strom Speicher per Postleitzahl: ${recordCount}`;
