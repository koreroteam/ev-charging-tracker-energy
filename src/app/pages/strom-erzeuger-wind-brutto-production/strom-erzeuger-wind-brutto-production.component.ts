...
@Component({
  selector: 'ngx-strom-erzeuger-wind-brutto-production',
  templateUrl: './strom-erzeuger-wind-brutto-production.component.html',
  styleUrls: ['./strom-erzeuger-wind-brutto-production.component.scss']
})
export class StromErzeugerWindBruttoProductionComponent implements OnInit, AfterViewInit {
  ...
  private createMap(): void {
    ...
    document.getElementById('heatMapContainerWindBruttoProduc').style.backgroundColor = "rgba(85,90,96,0.3)";
  
    this.map = L.map('heatMapContainerWindBruttoProduc', {
  ...
  private async getChargePointsDataZipCode(): Promise<any> {
    ...
    return { chargePointsData: mappedData, thresholds, totalRecordCountToday };
}
