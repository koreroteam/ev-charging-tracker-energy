...
@Component({
  selector: 'ngx-charging-infra-heat-map-area',
  templateUrl: './charging-infra-heat-map-area.component.html',
  styleUrls: ['./charging-infra-heat-map-area.component.scss']
})
export class ChargingInfraHeatMapAreaComponent implements OnInit, AfterViewInit {
  ...
  //  for (let district in chargePointsByKreise) {
  //   if (this.DISTRICT_SIZES_Kreise.has(district)) { 
  //     const count = chargePointsByKreise[district];
  //     const areaSize = this.DISTRICT_SIZES_Kreise.get(district);
  //     const chargePointDensity = count / areaSize;
  //     maxChargePointDensity = Math.max(maxChargePointDensity, chargePointDensity);
  //     minChargePointDensity = Math.min(minChargePointDensity, chargePointDensity);
  //   }
  // }
  ...
  this.http.get(kreiseDataUrl).subscribe((data: any) => {
        ...
        this.geoJsonLayer = L.geoJSON(data, {
          ...
          onEachFeature: (feature, layer) => {
            ...
            const areaSize = this.DISTRICT_SIZES_Kreise.get(kreiseName) ?? 1; 
            const chargePointDensity = chargePoints / areaSize;
            this.popupDataKreise.push([kreiseName, chargePoints, areaSize,  Number(chargePointDensity.toFixed(2))]);

  
            const popupContent = `Landkreis: ${kreiseName}<br>Anzahl Ladepunkte: ${chargePoints}<br>Fläche in km²: ${areaSize}<br>Ladepunkte pro km²: ${chargePointDensity.toFixed(2)}`;
      ...
      this.getRegierungsbezirkData().subscribe((data) => {
        ...
        this.geoJsonLayer = L.geoJSON(mergedGeoJSON, {
          ...
          onEachFeature: (feature, layer) => {
            ...
            const areaSize = this.DISTRICT_SIZES.get(name) ?? 1; 
            const chargePointDensity = count / areaSize;
            this.popupDataRegierungsbezirk.push([name, count, areaSize,  Number(chargePointDensity.toFixed(2))]);
  
            const popupContent = `Regierungsbezirk: ${name}<br>Anzahl Ladepunkte: ${count}<br>Fläche in km²: ${areaSize}<br>Ladepunkte pro km²: ${chargePointDensity.toFixed(2)}`;
...
private convertToCSV(data: any[], headerLabel: string): string {
  ...
  const header = [headerLabel, 'Anzahl Ladepunkte','Bezirk Fläche in km²','Ladepunkte pro km²'];
  let csvRows = [];
