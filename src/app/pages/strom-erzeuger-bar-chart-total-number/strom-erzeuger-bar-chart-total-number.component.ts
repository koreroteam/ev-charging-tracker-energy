...
@Component({
  selector: 'ngx-strom-erzeuger-bar-chart-total-number',
  templateUrl: './strom-erzeuger-bar-chart-total-number.component.html',
  styleUrls: ['./strom-erzeuger-bar-chart-total-number.component.scss']
})
export class StromErzeugerBarChartTotalNumberComponent implements OnInit {
  ...
  dataArray.forEach(item => {
    if (item && item.createdAtFormatted && item.stromErzeugerType) {
      ...
      if (aggregatedData[dateKey].hasOwnProperty(typeKey)) {
        aggregatedData[dateKey][typeKey] += item.recordCount;
      } else {
  ...
  if (apiType === 'combined') {
    ...
    filteredData.forEach(element => {
      ...
      switch (element.stromErzeugerType) {
        case 'Solar':
          solarData.push({ date: formattedDate, value: element.totalNumber });
          break;
        case 'Wind':
          windData.push({ date: formattedDate, value: element.totalNumber });
          break;
        case 'Strom Speicher':
          stromSpeicherData.push({ date: formattedDate, value: element.totalNumber });
          break;
      }
    });
    ...
    filteredData.forEach(element => { // Use filteredData here
      ...
      aggregatedData[formattedDate][apiType] += element.recordCount;
    });
