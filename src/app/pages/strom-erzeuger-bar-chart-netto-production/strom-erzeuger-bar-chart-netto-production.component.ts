...
@Component({
  selector: 'ngx-strom-erzeuger-bar-chart-netto-production',
  templateUrl: './strom-erzeuger-bar-chart-netto-production.component.html',
  styleUrls: ['./strom-erzeuger-bar-chart-netto-production.component.scss']
})
export class StromErzeugerBarChartNettoProductionComponent implements OnInit {
  ...
  dataArray.forEach(item => {
    if (item && item.createdAtFormatted && item.stromErzeugerType) {
      ...
      if (aggregatedData[dateKey].hasOwnProperty(typeKey)) {
        aggregatedData[dateKey][typeKey] += (item.nettonennLeistung / 1000) / 1000;// Convert kW to MW
      } else {
  ...
  if (apiType === 'combined') {
    ...
    filteredData.forEach(element => {
      ...
      switch (element.stromErzeugerType) {
        case 'Solar':
          solarData.push({ date: formattedDate, value: (element.totalNettoProduction / 1000) / 1000 }); // Convert kW to GW
        break;
          break;
        case 'Wind':
          windData.push({ date: formattedDate, value: (element.totalNettoProduction / 1000) / 1000  });
          break;
        case 'Strom Speicher':
          stromSpeicherData.push({ date: formattedDate, value: (element.totalNettoProduction / 1000) / 1000 });
          break;
      }
    });
    ...
    filteredData.forEach(element => { // Use filteredData here
      ...
      aggregatedData[formattedDate][apiType] += element.nettonennLeistung;
    });
